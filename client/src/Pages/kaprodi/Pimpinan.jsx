...
function Pimpinan() {
  ...
  const handleAction = async (e, actionType) => {
    ...
    if (actionType === "hapus") {
      dispatch(
        deleteData({ endpoint: `pimpinan/delete/${dataPimpinan.kode}` })
      );
      setIsDeleteModal(false);
    }

    if (actionType === "edit") {
      const selectedProperties = useSelectedProperties(dataPimpinan, [
        "nama",
        "jabatan",
      ]);
      const updatedData = {
      ...
      dispatch(
        updateData({
          endpoint: `pimpinan/update/${dataPimpinan.kode}`,
          data: updatedData,
        })
      );
      setIsModalEdit(false);
    ...
    <>
      ...
      <Modal className={"w-fit"} open={isModalEdit}>
        ...
        >
          <Input.TextInput
            type={"text"}
            label={"Nama"}
            value={dataPimpinan.nama}
            width={"full"}
            onChange={(e) => {
              handleInputValue(e, "nama", "edit");
            }}
          />
          <Input.TextInput
            value={dataPimpinan.jabatan}
            label={"Jabatan"}
            width={"full"}
            onChange={(e) => {
              handleInputValue(e, "jabatan", "edit");
            }}
          />
        </Modal.ModalCustom>
      </Modal>
      <Modal className={"w-fit"} open={isModalTambah}>
        ...
        >
          <Input.TextInput
            ...
            width={"full"}
            onChange={(e) => {
              handleInputValue(e, "nama", "tambah");
            }}
          />
          <Input.SelectInput
            value={kode}
            ...
            width={"full"}
            onChange={(e) => {
              handleInputValue(e, "kode", "tambah");
            }}
          />
          <Input.TextInput
            type={"text"}
            ...
            width={"full"}
            onChange={(e) => {
              handleInputValue(e, "jabatan", "tambah");
            }}
          />
        </Modal.ModalCustom>
      </Modal>
      <Modal open={isDeleteModal}>
        <Modal.ModalCustom
          ...
          title={`Apakah anda yakin ingin menghapus pimpinan '${dataPimpinan.nama}'?`}
