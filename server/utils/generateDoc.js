...
const document = async (id, sks_asal, sks_tujuan) => {
    const doc = new jsPDF({
    ...
    const mhs = await queriesStudents.findOne({
        id_mahasiswa: id
    });

    // Header
    doc.text("PERGURUAN TINGGI ASAL", 15, 25);
    doc.text(`Nama PT : ${mhs.dataValues.pt_asal}`, 15, 30);
    doc.text(`Fakultas : ${mhs.dataValues.fakultas}`, 15, 35);
    doc.text(`Prodi : ${mhs.dataValues.prodi}`, 15, 40);

    doc.text(`Nama Mahasiswa : ${mhs.dataValues.nama}`, 130, 25, null, null);
    doc.text(`NIM : ${mhs.dataValues.nim}`, 130, 30, null, null);
    doc.text(`Fakultas Tujuan : Ilmu Komputer`, 130, 35, null, null);
    doc.text(`Prodi Tujuan : ${mhs.dataValues.prodi_tujuan}`, 130, 40, null, null);
