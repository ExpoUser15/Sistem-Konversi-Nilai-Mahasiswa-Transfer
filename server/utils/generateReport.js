...
async function generatePDF(id, tmp, type) {
    const doc = new jsPDF({
    ...
    const dataPimpinan = await sequelize.query("SELECT * FROM tb_leaders");
    ...
    if (type === 'all') {
        ...
        doc.textWithLink('www.forlap.dikti.go.id', 63, doc.lastAutoTable.finalY + 8, { url: 'www.forlap.dikti.go.id' });
        ...
        doc.text("Diseleksi Oleh,", 153, 259, null, null, 'center');
        dataPimpinan[0].forEach(item => {
            if(item.kode === "4"){
                doc.text(item.nama, 153, 274, null, null, 'center');
            }
        })
    ...
    const query2 = await sequelize.query("SELECT tb_courses.id_mk, mk_asal, sks_asal, nilai_asal, nilai_tujuan, mata_kuliah FROM tb_conversions JOIN tb_courses ON tb_courses.id_mk = tb_conversions.id_mk WHERE id_mahasiswa = :id", {
    ...
    const dataPenempatan = await sequelize.query("SELECT MIN(penempatan) AS min_penempatan FROM tb_semesters WHERE id_mahasiswa = :id", {
        replacements: { id },
        type: QueryTypes.SELECT
    });
    ...
    hasil.push(["", "", "Total SKS", String(dataValues.total_sks_asal), "", "Total SKS", String(dataValues.total_sks_tujuan)]);
    ...
    doc.text(`: ${dataPenempatan[0].min_penempatan ? dataPenempatan[0].min_penempatan : ''}`, 105, finalY + 15);

    const tanggalRecap = await sequelize.query("SELECT tb_recapitulations.tanggal FROM tb_recapitulations WHERE id_mahasiswa = :id LIMIT 1", {
        replacements: { id },
        type: QueryTypes.SELECT
    });

    doc.text(`Sorong, ${getCurrentFormattedDate(tanggalRecap[0].tanggal).formatDate3}`, 105, finalY + 30, null, null, 'center');
    ...
    dataPimpinan[0].forEach(item => {
        if (item.kode === "1") {
            ...
            doc.text(item.jabatan, 55, finalY + 15, null, null, 'center');
            doc.text(item.nama, 55, finalY + 30, null, null, 'center');
        }

        if (item.kode === "2") {
            doc.text("Disetujui oleh,", 155, finalY + 10, null, null, 'center');
            doc.text(item.jabatan, 155, finalY + 15, null, null, 'center');
            doc.text(item.nama, 155, finalY + 30, null, null, 'center');
        }

        if (item.kode === "3") {
            doc.text("Mengetahui,", 105, finalY + 40, null, null, 'center');
            doc.text(item.jabatan, 105, finalY + 45, null, null, 'center');
            doc.text(item.nama, 105, finalY + 60, null, null, 'center');
        }
    });
    ...
    prosesData.dataPenempatan.forEach((item) => {
        const hasil = item.map(el => [
            el.mata_kuliah,
            el.sks
        ]);

        hasil.push([{ content: 'TOTAL', styles: { halign: 'right' } }, { content: item[item.length - 1].totalSKS, styles: { halign: 'left', fontStyle: 'bold' } }]);

        doc.text(`${item[0].penempatan % 2 === 0 ? 'Genap' : 'Ganjil'} ${item[0].penempatan}`, 100, lastY - 2);
    ...
    fs.writeFile(`./tmp/${tmp}/${filename}`, doc.output(), 'binary', async (err) => {
        if (err) {
